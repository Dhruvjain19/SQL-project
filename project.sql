...
/* select customers who have never bought any books from library*/
select custID, concat(fname, lname) as name
from customer
where custID not in (select distinct custID from transaction); 
/*select book with highest pagecount*/
select bkID, title from book where pageCount=(select max(pageCount)from book);
/* select customer who has borrowed some book for logest period*/
select customer.custID, concat(customer.fname, customer.lname) as name,datediff(return_date,issue_date) as days
from customer 
join transaction
on customer.custID=transaction.custID 
order by days desc limit 1;
/*display list of all books along withb their authors*/
select book.bkid, book.title, concat (author.fname,author.lname) as author
from book
left join author
on book.authorID=author.authorID ;
/* display list of top 20 most borrowed books*/
select book.bkID, book.title, count(transaction.bkID) as times_borrowed
from book
left join transaction
on book.bkID=transaction.bkID
group by book.bkID 
order by times_borrowed desc limit 20;
/* display list of top 10 customers who have  borrowed  most books*/
select customer.custID, concat(customer.fname, customer.lname) as name, count(transaction.custID) as books_borrowed
from customer
left join transaction
on customer.custID=transaction.custID
group by customer.custID 
order by books_borrowed desc limit 10;
